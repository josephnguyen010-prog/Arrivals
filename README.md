...
- **It reuses the spot-photo pipeline exactly.** Same 900px cap, same JPEG re-encode, same up-front
  ...
  `lib/spots.ts`. A full set of 80 replacements sits inside the budget.
...
The tab is only there when somebody has been. Sixty-two of the eighty cities have no entry,
...
- **Accounts and a real social graph.** The feed and the friends' notes are invented. Swapping
  ...
  is a real build.
- **More than 80 cities.** The catalogue is still a hand-written array; the real one is GeoNames.
...
All 80 default city photographs and the 26 spot photographs are CC0, public domain or
